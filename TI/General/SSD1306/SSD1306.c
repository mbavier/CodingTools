...
{ 
  ...
  uint8_t arguments;
  uint8_t commands = list[0];
  uint8_t commNum = 1;
  uint8_t commandString[16];
  // TWI: Init
  // -------------------------------------------------------------------------------------
  I2C_setupHelper();
  // TWI: start & SLAW
...
//   }
  ...
  while (commands--) {
    // Command
    // -----------------------------------------------------------------------------------
    commandString[0] = list[commNum++];
    // Arguments
    // -----------------------------------------------------------------------------------
    arguments = list[commNum++];
    uint8_t count = 1;
    while (arguments--) {
        commandString[count++] = list[commNum++];
    }
    I2C_WriteReg(I2C_0_INST, address, commandString[0], &commandString[1], count-1);
  }
  // TWI: Stop
  // -------------------------------------------------------------------------------------
 ...
 * @return  uint8_t
 */
uint8_t SSD1306_Send_Command (uint8_t address, uint8_t command)
{
  uint8_t status = INIT_STATUS;
  uint8_t commandString[1] = {command};
  I2C_WriteReg(I2C_0_INST, address, 0x80, commandString, 1);
...
uint8_t SSD1306_NormalScreen (uint8_t address)
{
  ...
  // send command
  // -------------------------------------------------------------------------------------   
  status = SSD1306_Send_Command (address, SSD1306_DIS_NORMAL);
  if (SSD1306_SUCCESS != status) {
    return status;
  }
...
uint8_t SSD1306_InverseScreen (uint8_t address)
{
  ...
  // send command
  // -------------------------------------------------------------------------------------   
  status = SSD1306_Send_Command (address, SSD1306_DIS_INVERSE);
  if (SSD1306_SUCCESS != status) {
    return status;
  }
...
uint8_t SSD1306_UpdateScreen (uint8_t address)
{
  ...
  I2C_WriteReg(I2C_0_INST, address, SSD1306_DATA_STREAM, cacheMemLcd, CACHE_SIZE_MEM);
...
//     cacheMemLcd[_counter++] = (char)(&FONTS[character-32][i++]);
//   }
//   _counter++;
